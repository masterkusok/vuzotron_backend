...
class BaseView(APIView):
    ...
    provider : ServiceProvider
        Instance of ServiceProvider inheritor. provider attribute
         is used to perform all db operations

    serializer_type : Type[Serializer]
        Type of serializer, which will be used to serialize responses
         to JSON
    """
    ...
    def _get_by_id(self, request: HttpRequest) -> JsonResponse:
        """
        Processes get request. Used to get serialized data about
         one specific object in db.
        ...
        if not id.isdigit():
            return JsonResponse(
                {"message": "Id must be a number"}, status=HTTPStatus.BAD_REQUEST
            )
    ...
    def _get_paginated_response(
        self, query: QuerySet, request: HttpRequest
    ) -> JsonResponse:
        """
    ...
    def post(self, request: HttpRequest) -> JsonResponse:
        ...
        if len(json_data) < len(self.provider.fields):
            return JsonResponse(
                {"message": "Missing fields"}, status=HTTPStatus.BAD_REQUEST
            )
        elif len(json_data) > len(self.provider.fields):
            return JsonResponse(
                {"message": "Too many fields"}, status=HTTPStatus.BAD_REQUEST
            )
        ...
        for required_field in self.provider.fields:
            ...
            if not json_data[required_field]:
                return JsonResponse(
                    {"message": f"Field {required_field} cannot be empty"},
                    status=HTTPStatus.BAD_REQUEST,
                )
            if not isinstance(
                json_data[required_field], self.provider.fields[required_field]
            ):
                return JsonResponse(
                    {
                        "message": f"Field {required_field} is should be type:"
                                   f" {self.provider.fields[required_field]}"
                    },
                    status=HTTPStatus.BAD_REQUEST,
                )
    ...
    def put(self, request: HttpRequest) -> JsonResponse:
        ...
        if not speciality:
            return JsonResponse(
                {"message": "Target id does not exist"}, status=HTTPStatus.NOT_FOUND
            )
        ...
        for required_field in json_data:
            if required_field not in self.provider.fields:
                return JsonResponse(
                    {"message": f"Unknown field {required_field}"},
                    status=HTTPStatus.BAD_REQUEST,
                )
            if not isinstance(
                json_data[required_field], self.provider.fields[required_field]
    ...
    def delete(self, request: HttpRequest) -> JsonResponse:
        ...
        return JsonResponse(
            {"message": "Error while deleting"}, status=HTTPStatus.BAD_REQUEST
        )
